...
or Scissors using a hand shape. Then, a winner for that round is selected: Rock 
...
with open('inputs/dec2.txt') as fin:
    while game := fin.readline().strip():
...
with open('inputs/dec2.txt') as fin:
    while game := fin.readline().strip():
        ...
        if outcome == 'X': # lose
            game_score = 0
            move_score = score_by_move[opponent_move] - 1
            if move_score == 0:
                move_score = 3 # opponent threw rock, i need to throw scissors
        elif outcome == 'Y': # tie
            game_score = 3
            move_score = score_by_move[opponent_move]
        else: # outcome == 'Z' # win
            game_score = 6
            move_score = score_by_move[opponent_move] + 1
            if move_score > 3:
                move_score = 1 # they throw scissor, i throw rock
