...
if __name__ == '__main__':
    ...
    with open('inputs/dec1.txt') as fin:
        while line := fin.readline():  
            if line == '\n':
                inventory.append([])
                idx += 1
            else:
                inventory[idx].append(int(line))
        
    calories = [sum(elf) for elf in inventory]
    max_calories = max(calories)
    print('Elf', calories.index(max_calories), 'is carrying', max_calories)
    ...
    print('The three largest elves are', calories[:3])
    print('The total amount of calories is', sum(calories[:3]) )